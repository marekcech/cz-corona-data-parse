...
function parseCorona() {
  ...
  // load data from regions
  
  var dataRegions = sheetRegions.getDataRange().getValues();
  
  APP.flush();
  Utilities.sleep(500);
  
  // if there are no data prepare the headers
  ...
  for (var d = 0; d < dataRegions.length; d++) {
    ...
    for (var r=0; r < data.infectedByRegion.length; r++) {
      ...
      if (data.lastUpdatedAtSource == dataRegions[d][0]) {
       ...
       rows.push([data.lastUpdatedAtSource,data.infectedByRegion[r].name, data.infectedByRegion[r].value]);
  ...
  if(rows[0]) { 
    ...
    sheetRegions.getRange(dataRegions.length,1, row, column).setValues(rows);
  }

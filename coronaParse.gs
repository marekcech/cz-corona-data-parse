...
function parseCorona() {
 ...
 rows.push(["date","tested cases", "positive tests"]);
  ...
  for (var v in data.numberOfTestedGraph) {
  var number = v+1;
    ...
    if (data.numberOfTestedGraph[v].date == dataSheet[number][0]) {
      
      continue;
     }
    }
    
    if (data.numberOfTestedGraph[v].date == data.totalPositiveTests[v].date) {
